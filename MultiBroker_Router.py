...
from fastapi import FastAPI, Body, BackgroundTasks, HTTPException, Depends
...
# ---------------------------------------------------------------------------
# Authentication and per‑user storage imports
#
# We import the auth router to mount it under /auth and pull in
# helper functions to decode JWTs and compute per‑user file paths.
...
from auth.github_store import gh_get_json, gh_put_json, gh_list_dir
...
# Dhan support has been removed. Only the Motilal broker is now supported.
MO_DIR = os.path.join(CLIENTS_ROOT, "motilal")
os.makedirs(MO_DIR, exist_ok=True)

# Retain a dummy DHAN_DIR constant for backward compatibility.  It is unused
# in the refactored code but prevents NameError if any legacy code paths
# reference it.
DHAN_DIR = ""
...
# Mount the authentication router under /auth.  This must come after
# the app and middleware have been created so that the auth routes are
# registered on the same application instance.
...
def _github_sync_down_all():
    # Sync down only the Motilal clients and other resources. Dhan has been removed.
    for rel in ("clients/motilal", "groups", "copy_setups"):
...
def _folder_for(broker: str) -> str:
    """Return the storage directory for the given broker.

    Dhan support has been removed, so this always returns the Motilal
    directory regardless of the broker name.
    """
    # Always use the Motilal directory since Dhan is no longer supported.
    return MO_DIR
...
def _save_minimal(broker: str, payload: Dict[str, Any]) -> str:
    ...
    # Dhan is no longer supported. Always treat new clients as Motilal
    # clients and normalize fields accordingly.
    ...
    doc = {
        ...
        # accept totpkey/mpin/otp from multiple places
        "totpkey": _pick(
            payload.get("totpkey"),
...
def _update_minimal(broker: str, payload: Dict[str, Any]) -> str:
    ...
    # Dhan is no longer supported; always treat edits as Motilal clients.
    creds = payload.get("creds") or {}
    doc = {
...
def _has_required_for_login(broker: str, c: Dict[str, Any]) -> bool:
    # Only Motilal clients are supported. Require the Motilal fields.
    return all((
...
def _dispatch_login(broker: str, path: str):
    try:
        ...
        # Only the Motilal broker is supported.
        mod_name = "Broker_motilal"
...
@app.get("/health")
def health():
    """Return the health status of available brokers.

    Only the Motilal broker is supported.
    """
    status = {}
    for key, mod_name in (("motilal","Broker_motilal"),):
        try:
...
def add_client(
    ...
    """
    Save a client under the currently authenticated user.

    The payload should include at least ``name`` and ``client_id`` (or
    ``userid``).  All other fields are stored verbatim and may include
    credentials such as password, pan, apikey, totpkey and capital.

    The client record is persisted under ``data/users/<userid>/clients/``
    both locally and in the configured GitHub repository.  After
    saving a background login task is triggered to authenticate the
    client with the Motilal broker.
    """
    # Derive name and client id from payload
    name = (payload.get("name") or payload.get("display_name") or "").strip()
    client_id = (payload.get("userid") or payload.get("client_id") or "").strip()
    if not name or not client_id:
        raise HTTPException(status_code=400, detail="name and client_id are required")
    # Bind owner and broker
    data = dict(payload)
    ...
    # Mirror to local filesystem so existing code reading from disk still works
    local_path = os.path.join(BASE_DIR, path.replace("/", os.sep))
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    with open(local_path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4)
    # Trigger background login asynchronously using injected BackgroundTasks
    background_tasks.add_task(_dispatch_login, "motilal", local_path)
...
def edit_client(
    ...
    """
    Edit an existing client record for the logged‑in user.

    The request payload should include the ``client_id`` (or ``userid``)
    identifying the client.  If the ``name`` is omitted the existing
    record cannot be located and an error is raised.  All other
    provided fields overwrite the stored values; missing fields are
    ...
    if not client_id or not name:
        raise HTTPException(status_code=400, detail="name and client_id are required")
    # Compute path and load existing record if any
    path = user_client_file(userid, name=name, client_id=client_id)
    existing, _ = gh_get_json(path)
    merged: Dict[str, Any] = {}
    if existing and isinstance(existing, dict):
        merged.update(existing)
    merged.update(payload)
    merged["owner_userid"] = userid
    merged.setdefault("broker", "motilal")
    gh_put_json(path, merged, message=f"edit_client {userid} {name} {client_id}")
    # Write locally
    local_path = os.path.join(BASE_DIR, path.replace("/", os.sep))
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    with open(local_path, "w", encoding="utf-8") as f:
        json.dump(merged, f, indent=4)
    return {"success": True, "message": "Client updated", "path": path}


@app.get("/clients")
def list_clients(userid: Optional[str] = Depends(get_current_user_optional)):
    """
    Return a list of clients for the effective user.

    If an Authorization header is present the token is used to
    determine the userid.  Otherwise the caller may supply a
    ``userid`` query parameter for backwards compatibility.  The
    returned structure mirrors the original behaviour but supports
    per‑user storage.
    """
    effective_user = normalize_userid(userid)
    if not effective_user:
        raise HTTPException(status_code=401, detail="Missing token (Authorization) or userid query param")
    # Attempt to list via GitHub first
    dir_path = user_clients_dir(effective_user)
    try:
        entries = gh_list_dir(dir_path)
    except HTTPException:
        # Fallback to local directory when GitHub is not configured
        local_dir = os.path.join(BASE_DIR, dir_path.replace("/", os.sep))
        entries = []
        if os.path.isdir(local_dir):
            for fn in os.listdir(local_dir):
                if fn.endswith(".json"):
                    entries.append({"type": "file", "path": f"{dir_path}/{fn}", "name": fn})
    ...
    for entry in entries:
        ...
        try:
            obj, _ = gh_get_json(p)
            if not obj and os.path.exists(os.path.join(BASE_DIR, p.replace("/", os.sep))):
                with open(os.path.join(BASE_DIR, p.replace("/", os.sep)), "r") as f:
                    obj = json.load(f)
            if isinstance(obj, dict):
                clients.append(obj)
        except Exception:
            pass
    # For backward compatibility include session_active boolean and broker
...
@app.get("/get_clients")
def get_clients_alias(userid: Optional[str] = Depends(get_current_user_optional)):
    """
    Legacy alias for ``/clients`` returning a simplified client list.

    The response contains only ``name``, ``client_id``, ``capital``
    and a human readable ``session`` field.
    """
    rows = list_clients(userid=userid)
    simplified = []
    for r in rows:
        simplified.append({
            "name": r.get("name", ""),
...
def delete_client(
    ...
    """
    Delete one or more clients for the logged‑in user.

    The request may specify a single client via ``client_id``/``userid``
    or a list of client identifiers under ``userids``.  The broker
    field is ignored as only the Motilal broker is supported.  For
    each identifier the server searches the user's clients directory
    for a file ending in ``_<client_id>.json`` and removes it from
    both the local filesystem and GitHub (if configured).
    """
    deleted: List[Dict[str, Any]] = []
    missing: List[Dict[str, Any]] = []
    # Build list of client ids to delete
    ids: List[str] = []
    if isinstance(payload.get("userids"), list):
        ids = [str(u).strip() for u in payload.get("userids") or [] if str(u).strip()]
    elif isinstance(payload.get("items"), list):
        for it in payload.get("items"):
            cid = str(it.get("userid") or it.get("client_id") or "").strip()
            if cid:
                ids.append(cid)
    else:
        cid = str(payload.get("userid") or payload.get("client_id") or "").strip()
        if cid:
            ids.append(cid)
    if not ids:
        raise HTTPException(status_code=400, detail="client_id(s) are required")
    # Local and remote deletion
    base_dir = os.path.join(BASE_DIR, user_clients_dir(userid).replace("/", os.sep))
    for cid in ids:
        found = False
        # search for any file ending with _<cid>.json
        if os.path.isdir(base_dir):
            for fn in os.listdir(base_dir):
                if fn.endswith(f"_{cid}.json"):
                    found = True
                    ...
                    try:
                        ...
                        raise HTTPException(status_code=500, detail=f"Failed deleting local file {fn}: {e}")
                    # delete from GitHub
                    rel_path = os.path.relpath(local_path, BASE_DIR).replace("\\", "/")
                    try:
                        _github_file_delete(rel_path)  # type: ignore[name-defined]
                    except Exception:
                        pass
                    deleted.append({"client_id": cid})
                    break
        if not found:
            missing.append({"client_id": cid, "reason": "not found"})
    return {"success": True, "deleted": deleted, "missing": missing}


@app.get("/debug/list_local_clients")
def debug_local_clients():
    """List the local client JSON files for debugging purposes.

    Only the Motilal broker is supported, so this returns a single-key dict.
    """
    result = {"motilal": []}
    for brk, folder in (("motilal", MO_DIR),):
        try:
...
def _guess_broker_from_order(order: Dict[str, Any]) -> str | None:
    """Determine which broker an order belongs to.

    Since Dhan is no longer supported, all orders are assumed to belong
    to the Motilal broker regardless of their order_id format.
    """
    # Always return 'motilal' as the target broker.
    return "motilal"
...
def _broker_by_client_name(name: str) -> str | None:
    ...
    # Only search the Motilal directory; Dhan support removed.
    for brk, folder in (("motilal", MO_DIR),):
        try:
            for fn in os.listdir(folder):
                if not fn.endswith('.json'):
...
def route_cancel_order(payload: Dict[str, Any] = Body(...)):
    ...
    # Only Motilal broker is supported.
...
def route_close_positions(payload: Dict[str, Any] = Body(...)):
    ...
    # Only keep a bucket for Motilal. Dhan support removed.
    buckets = {"motilal": []}
    for it in items:
    ...
    for brk, rows in buckets.items():
        if not rows:
            continue
        try:
            # Always import the Motilal broker for closing positions.
            mod = importlib.import_module("Broker_motilal")
...
def route_place_orders(payload: Dict[str, Any] = Body(...)):
    ...
    security_id_raw = parts[2] if len(parts) > 2 else ""
    symboltoken_raw = parts[3] if len(parts) > 3 else ""
    ...
    # ... rest of your function continues here (still indented 4 spaces)
    ...
    BASE_DIR   = os.path.abspath(os.environ.get("DATA_DIR", "./data"))
    # Define only the Motilal clients directory; Dhan is not supported.
    DHAN_DIR   = os.path.join(BASE_DIR, "clients", "dhan")  # retained for backward compatibility (unused)
    ...
    # ------------------- bucket by broker -------------------
    # Only the Motilal broker is supported; initialize a single bucket.
    by_broker: Dict[str, List[Dict[str, Any]]] = {"motilal": []}
    ...
    # Dhan support has been removed: skip lot-size multiplication for Dhan.
...
def route_modify_order(payload: Dict[str, Any] = Body(...)):
    ...
    def _guess_broker_from_order(od: Dict[str, Any]) -> str | None:
        # With Dhan removed, always return 'motilal' regardless of order_id format.
        return "motilal"
