...
# Import JWT helpers from the auth package.  These are used in the
# require_user helper below to support Authorization: Bearer <token>
try:
    from auth.auth_utils import jwt_decode, SECRET_KEY  # type: ignore
except Exception:
    # If auth package is not present the JWT helpers will not be used.
    jwt_decode = None  # type: ignore
...
# Mount auth router if present.  Mount under /auth so that routes
# like /auth/register and /auth/login exist.  Without a prefix the
# auth routes would otherwise live at the root.
...
def require_user(user_id: Optional[str], authorization: Optional[str] = None) -> str:
    """Return a user identifier from either an auth token or header.

    If an Authorization header containing a Bearer token is provided and
    the auth package is available, the token will be decoded and the
    embedded ``userid`` claim returned.  If decoding fails a 401
    (unauthorised) error is raised.  If no valid token is present,
    the function falls back to the ``X-User-Id`` header.  If that
    header is missing or empty a 400 error is raised.
    """
    # First attempt to extract userid from a JWT if provided and supported
    if authorization:
        # Expect formats like "Bearer <token>" or just the token
        token = authorization.strip()
        parts = token.split()
        if len(parts) == 2 and parts[0].lower() == "bearer":
            token = parts[1]
        # Only attempt to decode if jwt helpers are available
        if jwt_decode and SECRET_KEY and token:
            try:
                ...
                # If token is provided but invalid we return 401
...
async def clients_add(
    ...
    uid = require_user(user_id, authorization)
    ...
    # Persist client in a simple format (name, userid, credentials, capital).
    creds = payload.get("creds") or {}
    password = (payload.get("password") or creds.get("password") or "").strip()
    pan = (payload.get("pan") or creds.get("pan") or "").strip()
    apikey = (payload.get("apikey") or creds.get("apikey") or "").strip()
    totpkey = (
        payload.get("totpkey")
        or creds.get("totpkey")
        or creds.get("totp_key")
        or ""
    ).strip()
    capital = payload.get("capital", "")
    try:
        capital = int(capital) if capital not in ("", None) else ""
    except Exception:
        pass
